...
Compute the cost generated by `mycost` given the variables `vars`. The return
value must be a scalar.

!!! note
    Required user-specialized API function.
"""
function computecost end


"""
    NLLSsolver.computecostgradhess(varflags, mycost, vars...)

Compute the cost generated by `mycost`, and its first and second derivatives w.r.t. the 
variables indicated by the set bits in varflags (the first bit corresponds to the first 
variable, and so on) given the variables `vars`. The return values must be a scalar, vector 
and matrix.
...
!!! note
    Optional user-specialized API function.
"""
function computecostgradhess end
